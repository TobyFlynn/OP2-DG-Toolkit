...
void add_3d_vec_solution(File *file, const std::string &nameX, const std::string &nameY,
                         ...
                         vtkUnstructuredGrid *vtkGrid, const std::vector<bool> &use_cell, const int activeCells) {
  if(file->exist(nameX) && file->exist(nameY) && file->exist(nameZ)) {
    ...
    sol_vector->SetNumberOfTuples(activeCells * DG_NP);
    int count = 0;
    for(int i = 0; i < numCells; i++) {
      if(use_cell[i]) {
        for(int j = 0; j < DG_NP; j++) {
          sol_vector->SetTuple3(count * DG_NP + j, u_vec[i][j], v_vec[i][j], w_vec[i][j]);
        }
        count++;
      }
    }
...
void add_2d_vec_solution(File *file, const std::string &nameX, const std::string &nameY,
  ...
  if(file->exist(nameX) && file->exist(nameY)) {
    ...
    for(int i = 0; i < numCells; i++) {
      if(use_cell[i]) {
        for(int j = 0; j < DG_NP; j++) {
          sol_vector->SetTuple2(count * DG_NP + j, u_vec[i][j], v_vec[i][j]);
        }
        count++;
      }
    }
...
void add_1d_vec_solution(File *file, const std::string &name, const std::string &nameVec,
                         const int numCells, vtkUnstructuredGrid *vtkGrid, const std::vector<bool> &use_cell, const int activeCells) {
  if(file->exist(name)) {
    ...
    for(int i = 0; i < numCells; i++) {
      if(use_cell[i]) {
        for(int j = 0; j < DG_NP; j++) {
          sol_vector->SetTuple1(count * DG_NP + j, u_vec[i][j]);
        }
        count++;
      }
    }
...
int main(int argc, char **argv) {
  ...
  for(int arg = 1; arg < argc; arg++) {
    ...
    std::vector<bool> use_cell;
    int activeCells = 0;

    // Set points
    for(int cell = 0; cell < numCells; cell++) {
      if(x_vec[cell][0] < X_LIMIT) {
        use_cell.push_back(true);
        activeCells++;
        for(int i = 0; i < DG_NP; i++) {
          vtkPoints->InsertNextPoint(x_vec[cell][i], y_vec[cell][i], z_vec[cell][i]);
        }
      } else {
        use_cell.push_back(false);
      }
      // for(int i = 0; i < DG_NP; i++) {
      //   vtkPoints->InsertNextPoint(x_vec[cell][i], y_vec[cell][i], z_vec[cell][i]);
      // }
    ...
    for(int cell = 0; cell < numCells; cell++) {
      if(use_cell[cell]) {
        for(int i = 0; i < numSubCells; i++) {
          const int basePtInd = count * DG_NP;
    ...
    add_3d_vec_solution(&file, "ins_solver_vel00", "ins_solver_vel01",
                        "ins_solver_vel02", "velocity", numCells, vtkGrid, use_cell, activeCells);
    add_3d_vec_solution(&file, "ins_solver_vel10", "ins_solver_vel11",
                        "ins_solver_vel12", "velocity", numCells, vtkGrid, use_cell, activeCells);
    add_3d_vec_solution(&file, "ins_solver_velT0", "ins_solver_velT1",
                        "ins_solver_velT2", "velocityT", numCells, vtkGrid, use_cell, activeCells);
    add_3d_vec_solution(&file, "ins_solver_velTT0", "ins_solver_velTT1",
                        "ins_solver_velTT2", "velocityTT", numCells, vtkGrid, use_cell, activeCells);
    add_1d_vec_solution(&file, "ins_solver_pr", "Pressure", numCells, vtkGrid, use_cell, activeCells);
    add_1d_vec_solution(&file, "ins_solver_rho", "Rho", numCells, vtkGrid, use_cell, activeCells);
    add_1d_vec_solution(&file, "ins_solver_mu", "Mu", numCells, vtkGrid, use_cell, activeCells);
    add_1d_vec_solution(&file, "ls_solver_s", "Surface", numCells, vtkGrid, use_cell, activeCells);
    add_1d_vec_solution(&file, "ins_solver_shock_cap_art_vis", "Artificial Vis", numCells, vtkGrid, use_cell, activeCells);

    add_3d_vec_solution(&file, "advec_u", "advec_v", "advec_w", "velocity", numCells, vtkGrid, use_cell, activeCells);
    add_1d_vec_solution(&file, "advec_val", "Val", numCells, vtkGrid, use_cell, activeCells);
