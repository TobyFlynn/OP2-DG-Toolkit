inline void div_weak_3d_central_flux(const int *faceNum, const int *fmaskL_corrected,
  ...
  for(int i = 0; i < DG_NPF; i++) {
    ...
    double fluxU = 0.5 * (u[0][fmaskL[i]] + u[1][fmaskR_corrected[i]]);
    double fluxV = 0.5 * (v[0][fmaskL[i]] + v[1][fmaskR_corrected[i]]);
    double fluxW = 0.5 * (w[0][fmaskL[i]] + w[1][fmaskR_corrected[i]]);
    flux[0][find] += fscale[0] * (nx[0] * fluxU + ny[0] * fluxV + nz[0] * fluxW);
  }

  for(int i = 0; i < DG_NPF; i++) {
    int find = faceNum[1] * DG_NPF + i;
    double fluxU = 0.5 * (u[0][fmaskL_corrected[i]] + u[1][fmaskR[i]]);
    double fluxV = 0.5 * (v[0][fmaskL_corrected[i]] + v[1][fmaskR[i]]);
    double fluxW = 0.5 * (w[0][fmaskL_corrected[i]] + w[1][fmaskR[i]]);
