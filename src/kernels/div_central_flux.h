inline void div_central_flux(const int *edgeNum, const bool *rev,
  ...
  for(int i = 0; i < DG_GF_NP; i++) {
    ...
    double flux_u = u[0][lInd] - 0.5 * (u[0][lInd] + u[1][rInd]);
    double flux_v = v[0][lInd] - 0.5 * (v[0][lInd] + v[1][rInd]);
  ...
  for(int i = 0; i < DG_GF_NP; i++) {
    ...
    double flux_u = u[1][rInd] - 0.5 * (u[0][lInd] + u[1][rInd]);
    double flux_v = v[1][rInd] - 0.5 * (v[0][lInd] + v[1][rInd]);
