...
__global__ void jump_3d_kerenel(
...
void DGMesh3D::jump(op_dat in, op_dat out) {
  ...
  if (OP_diags>2) {
    printf(" kernel routine with indirection: jump_3d\n");
  }
  ...
  if (set_size > 0) {
    ...
    for ( int round=0; round<2; round++ ){
      ...
      if (end-start>0) {
        ...
        jump_3d_kerenel<<<nblocks,nthread>>>(
...
__global__ void jump_3d_sp_kerenel(
  const int *__restrict map,
...
void DGMesh3D::jump_sp(op_dat in, op_dat out) {
  ...
  if (set_size > 0) {
    ...
    for ( int round=0; round<2; round++ ){
      ...
      if (end-start>0) {
        int nblocks = (end-start-1)/nthread+1;
        jump_3d_sp_kerenel<<<nblocks,nthread>>>(
          node2node_custom_map_d,
          (float *)in_arg.data_d,
