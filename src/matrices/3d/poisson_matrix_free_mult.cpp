...
void PoissonMatrixFreeMult3D::check_current_order() {
...
void PoissonMatrixFreeMult3D::calc_tau() {
...
void PoissonMatrixFreeMult3D::mat_free_mult(op_dat in, op_dat out) {
  timer->startTimer("PoissonMatrixFreeMult3D " + std::to_string(mesh->order_int) + " - mult");
  check_current_order();
  ...
  timer->startTimer("PoissonMatrixFreeMult3D " + std::to_string(mesh->order_int) + " - mult grad");
  ...
  timer->endTimer("PoissonMatrixFreeMult3D " + std::to_string(mesh->order_int) + " - mult grad");
  ...
  timer->endTimer("PoissonMatrixFreeMult3D " + std::to_string(mesh->order_int) + " - mult faces");

  timer->startTimer("PoissonMatrixFreeMult3D " + std::to_string(mesh->order_int) + " - mult bfaces");
  if(mesh->bface2cells) {
  ...
  timer->endTimer("PoissonMatrixFreeMult3D " + std::to_string(mesh->order_int) + " - mult bfaces");

  timer->startTimer("PoissonMatrixFreeMult3D " + std::to_string(mesh->order_int) + " - finish flux");
  ...
  timer->endTimer("PoissonMatrixFreeMult3D " + std::to_string(mesh->order_int) + " - mult cells MM");
  timer->startTimer("PoissonMatrixFreeMult3D " + std::to_string(mesh->order_int) + " - mult cells Emat");
  ...
  timer->endTimer("PoissonMatrixFreeMult3D " + std::to_string(mesh->order_int) + " - mult cells Emat");
  timer->startTimer("PoissonMatrixFreeMult3D " + std::to_string(mesh->order_int) + " - mult cells geof");
  ...
  timer->endTimer("PoissonMatrixFreeMult3D " + std::to_string(mesh->order_int) + " - mult cells geof");
  timer->startTimer("PoissonMatrixFreeMult3D " + std::to_string(mesh->order_int) + " - mult cells D");
  ...
  timer->endTimer("PoissonMatrixFreeMult3D " + std::to_string(mesh->order_int) + " - mult cells D");
  timer->endTimer("PoissonMatrixFreeMult3D " + std::to_string(mesh->order_int) + " - cells");
  ...
  timer->endTimer("PoissonMatrixFreeMult3D " + std::to_string(mesh->order_int) + " - mult");
}

void PoissonMatrixFreeMult3D::mat_free_mult_sp(op_dat in, op_dat out) {
  timer->startTimer("PoissonMatrixFreeMult3D sp" + std::to_string(mesh->order_int) + " - mult");
  #ifndef NEW_MAT_MULT_KERNEL
  check_current_order();
  #endif
  ...
  timer->startTimer("PoissonMatrixFreeMult3D sp" + std::to_string(mesh->order_int) + " - mult grad");
  // mesh->grad(in, tmp_grad0.dat, tmp_grad1.dat, tmp_grad2.dat);
  // mesh->grad_halo_exchange(in, tmp_grad0.dat, tmp_grad1.dat, tmp_grad2.dat);
  ...
  timer->endTimer("PoissonMatrixFreeMult3D sp" + std::to_string(mesh->order_int) + " - mult grad");
  ...
  #ifdef NEW_MAT_MULT_KERNEL
  timer->startTimer("PoissonMatrixFreeMult3D sp" + std::to_string(mesh->order_int) + " - indirect");
  op_par_loop(pmf_3d_mult_indirect_sp, "pmf_3d_mult_indirect_sp", mesh->faces,
              op_arg_gbl(&mesh->order_int, 1, "int", OP_READ),
              op_arg_dat(mesh->faceNum, -1, OP_ID, 2, "int", OP_READ),
              op_arg_dat(mesh->fmaskL,  -1, OP_ID, DG_NPF, "int", OP_READ),
              op_arg_dat(mesh->fmaskR,  -1, OP_ID, DG_NPF, "int", OP_READ),
              ...
              op_arg_dat(mesh->sJ, -1, OP_ID, 2, DG_FP_STR, OP_READ),
              op_arg_dat(mesh->fscale, -1, OP_ID, 2, DG_FP_STR, OP_READ),
              op_arg_dat(in, -2, mesh->face2cells, DG_NP, "float", OP_READ),
              op_arg_dat(tmp_grad0.dat, -2, mesh->face2cells, DG_NP, "float", OP_READ),
              op_arg_dat(tmp_grad1.dat, -2, mesh->face2cells, DG_NP, "float", OP_READ),
              op_arg_dat(tmp_grad2.dat, -2, mesh->face2cells, DG_NP, "float", OP_READ),
              op_arg_dat(tmp_npf0.dat, -2, mesh->face2cells, DG_NUM_FACES * DG_NPF, "float", OP_WRITE),
              op_arg_dat(tmp_npf1.dat, -2, mesh->face2cells, DG_NUM_FACES * DG_NPF, "float", OP_WRITE),
              op_arg_dat(tmp_npf2.dat, -2, mesh->face2cells, DG_NUM_FACES * DG_NPF, "float", OP_WRITE),
              op_arg_dat(tmp_npf3.dat, -2, mesh->face2cells, DG_NUM_FACES * DG_NPF, "float", OP_WRITE));

  if(mesh->bface2cells) {
    throw std::runtime_error("TODO");
  }
  timer->endTimer("PoissonMatrixFreeMult3D sp" + std::to_string(mesh->order_int) + " - indirect");
  #else
  timer->startTimer("PoissonMatrixFreeMult3D sp" + std::to_string(mesh->order_int) + " - indirect");
  timer->startTimer("PoissonMatrixFreeMult3D sp" + std::to_string(mesh->order_int) + " - mult faces");
  ...
  timer->endTimer("PoissonMatrixFreeMult3D sp" + std::to_string(mesh->order_int) + " - mult faces");

  timer->startTimer("PoissonMatrixFreeMult3D sp" + std::to_string(mesh->order_int) + " - mult bfaces");
  if(mesh->bface2cells) {
  ...
  timer->endTimer("PoissonMatrixFreeMult3D sp" + std::to_string(mesh->order_int) + " - mult bfaces");

  timer->startTimer("PoissonMatrixFreeMult3D sp" + std::to_string(mesh->order_int) + " - finish flux");
  ...
  timer->endTimer("PoissonMatrixFreeMult3D sp" + std::to_string(mesh->order_int) + " - finish flux");
  timer->endTimer("PoissonMatrixFreeMult3D sp" + std::to_string(mesh->order_int) + " - indirect");
  #endif
  timer->startTimer("PoissonMatrixFreeMult3D sp" + std::to_string(mesh->order_int) + " - cells");
  timer->startTimer("PoissonMatrixFreeMult3D sp" + std::to_string(mesh->order_int) + " - mult cells MM");
  mesh->mass_sp(tmp_grad0.dat);
  mesh->mass_sp(tmp_grad1.dat);
  mesh->mass_sp(tmp_grad2.dat);
  timer->endTimer("PoissonMatrixFreeMult3D sp" + std::to_string(mesh->order_int) + " - mult cells MM");
  timer->startTimer("PoissonMatrixFreeMult3D sp" + std::to_string(mesh->order_int) + " - mult cells Emat");
  ...
  timer->endTimer("PoissonMatrixFreeMult3D sp" + std::to_string(mesh->order_int) + " - mult cells Emat");
  timer->startTimer("PoissonMatrixFreeMult3D sp" + std::to_string(mesh->order_int) + " - mult cells geof");
  ...
  timer->endTimer("PoissonMatrixFreeMult3D sp" + std::to_string(mesh->order_int) + " - mult cells geof");
  timer->startTimer("PoissonMatrixFreeMult3D sp" + std::to_string(mesh->order_int) + " - mult cells D");
  ...
  timer->endTimer("PoissonMatrixFreeMult3D sp" + std::to_string(mesh->order_int) + " - mult cells D");
  timer->endTimer("PoissonMatrixFreeMult3D sp" + std::to_string(mesh->order_int) + " - cells");
  ...
  timer->endTimer("PoissonMatrixFreeMult3D sp" + std::to_string(mesh->order_int) + " - mult");
}
