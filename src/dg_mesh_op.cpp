...
void op_par_loop_cub_mm_init(char const *, op_set,
  op_arg,
  op_arg,
  op_arg,
  op_arg );
...
void op_par_loop_init_order(char const *, op_set,
  op_arg );
...
void op_par_loop_interp_dat_to_new_order_int(char const *, op_set,
  op_arg,
  op_arg,
  op_arg,
  op_arg );

void op_par_loop_copy_new_orders_int(char const *, op_set,
  op_arg,
  op_arg );

void op_par_loop_interp_dat_to_max_order(char const *, op_set,
  op_arg,
  op_arg,
  op_arg,
  op_arg );
...
void DGCubatureData::update_mesh_constants() {
  ...
  op_par_loop_cub_mm_init("cub_mm_init",mesh->cells,
              op_arg_dat(mesh->order,-1,OP_ID,1,"int",OP_READ),
              op_arg_gbl(cubV_g,DG_ORDER * DG_CUB_NP * DG_NP,"double",OP_READ),
...
DGMesh::DGMesh(double *coords_a, int *cells_a, int *edge2node_a,
  ...
  set_cuda_const();
  #else
  op_decl_const2("DG_CONSTANTS",DG_ORDER * 5,"int",DG_CONSTANTS);
  op_decl_const2("FMASK",DG_ORDER * DG_NPF * 3,"int",FMASK);
  op_decl_const2("cubW_g",DG_ORDER * DG_CUB_NP,"double",cubW_g);
  op_decl_const2("gaussW_g",DG_ORDER * DG_GF_NP,"double",gaussW_g);
  #endif
...
void DGMesh::init() {
  ...
  op_par_loop_init_order("init_order",cells,
...
void DGMesh::update_order(int new_order,
  ...
  for(int i = 0; i < dats_to_interpolate.size(); i++) {
    ...
    op_par_loop_interp_dat_to_new_order_int("interp_dat_to_new_order_int",cells,
                op_arg_gbl(order_interp_g,DG_ORDER * DG_ORDER * DG_NP * DG_NP,"double",OP_READ),
                op_arg_dat(order,-1,OP_ID,1,"int",OP_READ),
  ...
  // Copy across new orders
  op_par_loop_copy_new_orders_int("copy_new_orders_int",cells,
              op_arg_gbl(&new_order,1,"int",OP_READ),
              op_arg_dat(order,-1,OP_ID,1,"int",OP_WRITE));
...
void DGMesh::interp_to_max_order(std::vector<op_dat> &dats_in,
  ...
  for(int i = 0; i < dats_in.size(); i++) {
    ...
    op_par_loop_interp_dat_to_max_order("interp_dat_to_max_order",cells,
                op_arg_gbl(order_interp_g,DG_ORDER * DG_ORDER * DG_NP * DG_NP,"double",OP_READ),
                op_arg_dat(order,-1,OP_ID,1,"int",OP_READ),
